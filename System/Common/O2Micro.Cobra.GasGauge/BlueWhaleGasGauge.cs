...
namespace O2Micro.Cobra.GasGauge
{
	...
	public class BlueWhaleGasGauge : GasGaugeInterface
    {
		...
		private enum prShortIndex : byte
		{
			...
			iStatus					= 0x15,			//save OZ8806 status register value
            iOCVbit                 = 0x16,			//in bool, 1 : PoOCV, 2 is SleepOCV, 0 is error or default
		...
		private enum prFloatIndex : byte
		{
			...
			iRSOC					= 0x05,			//in %, save RelativeStateOfCharge
			...
			//iOCVbit					= 0x09,			//in bool, 1 : PoOCV, 2 is SleepOCV, 0 is error or default
            ...
            ///////////Leon: Add Ubat and U0
            iUbat = 0x20,                       //Ubat= I*R1 + Uadc
            iU0 = 0x21,                         //U0 = I*R0 + Ubat
		}
        ...
        private enum OZ8806Register : byte
        {
			RegTrimRsv = 0x03,
			RegFreeze = 0x04,
			RegLDOTrim = 0x08,
		...
		private enum ChargerRegister : byte
		{
			RegChargeCurrent = 0x90,
			RegVBusStatus = 0xC0,
			RegChargerStatus = 0xC1,
		}
		...
		*/
		...
		struct config_data_t
		{
			...
			//byte debug;                                          // enable or disable O2MICRO debug information
		...
		struct gas_gauge_t
		{
			...
			public Int32 charge_ratio;  //this must be static 
			...
			public Int32 discharge_ratio;  //this must be static 
		...
		//private byte yOZ8806SubSec = 2;		//subsection definition, same as XML, used in DEM
		//private byte yOZSeaElfSubSec = 3;	//sunsection definition, same as XML, used in DEM
        ...
        //private float Rcon = 0;
        //private float Rbat = 0;

        //Android Driver variable
        private string VERSION = "2015.7.20.1/6.00.01";
        ...
        private string FLASH_TEST = string.Format("flash_test.dat");
        private string CM_PATH = string.Format("oz8806api");
        ...
        //private Parameter ParamOZ8806Status = null;
		private Parameter ParamOZ8806Regx03Bit7 = null;
		private Parameter ParamOZ8806Regx04Bit0 = null;
		private Parameter ParamOZ8806Regx08Bit0 = null;
		private Parameter ParamOZ8806CtrlBI = null;
		private Parameter ParamOZ8806Ctrlvme = null;
		//private Parameter ParamOZ8806CtrlChgActive = null;
		private Parameter ParamOZ8806CtrlSlpOCVEn = null;
		private Parameter ParamOZ8806CtrlSleepMode = null;
		private Parameter ParamOZ8806CtrlSWReset = null;
        private Parameter ParamOZ8806OCV = null;
		private Parameter ParamOZ8806PoOCV = null;
		private Parameter ParamOZ8806SleepOCV = null;
        private Parameter ParamOZ8806BoardOffset = null;
		private Parameter ParamChargerChargeCurr = null;
		private Parameter ParamChargerVBusOK = null;
		private Parameter ParamChargerCCState = null;
		private Parameter ParamChargerCVState = null;
		private Parameter ParamChargerChgFull = null;
        #endregion
        ...
        //private byte    oz8806_pec_check  = 0;
        private byte    oz8806_cell_num  = 1;
        ...
        private byte    oz8806_in_suspend = 0;
		...
		
		{
            ...
            try
            {
                ...
                if (projtable == null)
                {
                    ...
                    //string tmpDLLFile = new string("D:\\O2Micro\\Project - Cobra\\SourceCode\\SW_Cobra\\output\\O2Micro.Cobra.GasGauge.dll".ToCharArray());
                ...
                bInit = myProject.InitializeProject(ref uGGErrorcode, false, true);
                if (!bInit)
                {
                    return bInit;
                }
                ...
                //else
                //{
                    //foreach(Parameter pmr in PListGGSetting)
                    //{
		...
		public bool CalculateGasGauge()
		{
			...
			/* report to system, don't need
			//if(adapter_status != 0)
			//{
				//if (batt_info.fRSOC >= 100 && get_charger_full_status()
						//(oz8806_battery_get_qpnp_bat_prop(data, POWER_SUPPLY_PROP_STATUS) !=
						//POWER_SUPPLY_STATUS_FULL)) {
				//{
					//ret.intval = POWER_SUPPLY_STATUS_FULL;
					//data.qpnp_bat.set_property(data.qpnp_bat, POWER_SUPPLY_PROP_STATUS, &ret);
			...
			// cannot modify polling time, skip this.
			//if (!bmu_init_ok) {
		...
		private bool gdm_init()
		{
			...
			foreach (Parameter pmGGP in PListGGPolling)
			{
                ...
                switch(u16Addr)
                {
                    case (UInt16)OZ8806Register.RegCellVolt:
                        {
                            ParamPhyVoltage = pmGGP;
                            break;
                        }
                    case (UInt16)OZ8806Register.RegCellCurr:
                        {
                            ParamPhyCurrent = pmGGP;
                            break;
                        }
                    case (UInt16)OZ8806Register.RegCellTemp:
                        {
                            ParamPhyTemperature = pmGGP;
                            break;
                        }
                    case (UInt16)OZ8806Register.RegCellCAR:
                        {
                            ParamPhyCAR = pmGGP;
                            break;
                        }
			...
			foreach (Parameter pmGGS in PListGGSetting)
			{
				...
				switch (u16Addr)
				{
					case (UInt16)OZ8806Register.RegTrimRsv:
						{
							foreach (KeyValuePair<string, Reg> tmpreg in pmGGS.reglist)
							{
								if (tmpreg.Key.Equals("Low"))
								{
									...
									{
										ParamOZ8806Regx03Bit7 = pmGGS;
									}
								}
							}
							break;
						}
					case (UInt16)OZ8806Register.RegFreeze:
						{
							foreach (KeyValuePair<string, Reg> tmpreg in pmGGS.reglist)
							{
								if (tmpreg.Key.Equals("Low"))
								{
									...
									{
										ParamOZ8806Regx04Bit0 = pmGGS;
									}
								}
							}
							break;
						}
					case (UInt16)OZ8806Register.RegLDOTrim:
						{
							foreach (KeyValuePair<string, Reg> tmpreg in pmGGS.reglist)
							{
								if (tmpreg.Key.Equals("Low"))
								{
									if (tmpreg.Value.startbit == 0)
									{
										ParamOZ8806Regx08Bit0 = pmGGS;
									}
								}
							}
							break;
						}
					case (UInt16)OZ8806Register.RegStatus:
						{
							//ParamOZ8806Status = pmGGS;
							foreach (KeyValuePair<string, Reg> tmpreg in pmGGS.reglist)
							{
								if (tmpreg.Key.Equals("Low"))
								{
									if (tmpreg.Value.startbit == 0)
									{
										ParamOZ8806CtrlBI = pmGGS;
									}
									else if (tmpreg.Value.startbit == 2)
									{
										ParamOZ8806Ctrlvme = pmGGS;
									}
									else if (tmpreg.Value.startbit == 4)
									{
										//ParamOZ8806CtrlChgActive = pmGGS;
									}
									else if (tmpreg.Value.startbit == 5)
									{
										ParamOZ8806CtrlSlpOCVEn = pmGGS;
									}
									else if (tmpreg.Value.startbit == 6)
									{
										ParamOZ8806CtrlSleepMode = pmGGS;
									}
									else if (tmpreg.Value.startbit == 7)
									{
										ParamOZ8806CtrlSWReset = pmGGS;
									}
								}
							}
							break;
						}
					case (UInt16)OZ8806Register.RegCellOCV:
						{
							foreach (KeyValuePair<string, Reg> tmpreg in pmGGS.reglist)
							{
								if (tmpreg.Key.Equals("Low"))
								{
									if (tmpreg.Value.startbit == 0)
									{
										ParamOZ8806PoOCV = pmGGS;
									}
									else if (tmpreg.Value.startbit == 1)
									{
										ParamOZ8806SleepOCV = pmGGS;
									}
									else if (tmpreg.Value.startbit == 4)
									{
										ParamOZ8806OCV = pmGGS;
									}
								}
							}
							break;
						}
                    case (UInt16)OZ8806Register.RegBoardOffset:
                        {
                            ParamOZ8806BoardOffset = pmGGS;
                            break;
                        }
					case (UInt16)ChargerRegister.RegChargeCurrent:
						{
							ParamChargerChargeCurr = pmGGS;
							break;
						}
					case (UInt16)ChargerRegister.RegVBusStatus:
						{
							foreach (KeyValuePair<string, Reg> tmpreg in pmGGS.reglist)
							{
								if (tmpreg.Key.Equals("Low"))
								{
									if (tmpreg.Value.startbit == 5)
									{
										ParamChargerVBusOK = pmGGS;
									}
								}
							}
							break;
						}
					case (UInt16)ChargerRegister.RegChargerStatus:
						{
							foreach (KeyValuePair<string, Reg> tmpreg in pmGGS.reglist)
							{
								if (tmpreg.Key.Equals("Low"))
								{
									if (tmpreg.Value.startbit == 2)
									{
										ParamChargerCCState = pmGGS;
									}
									else if (tmpreg.Value.startbit == 3)
									{
										ParamChargerCVState = pmGGS;
									}
									else if (tmpreg.Value.startbit == 4)
									{
										ParamChargerChgFull = pmGGS;
									}
								}
							}
							break;
			...
			if (ParamChargerChargeCurr.phydata == 0)
				bGDM = false;
		...
		private bool GetOCVValue()
		{
			...
			if (ParamOZ8806OCV != null)
			{
				batt_info.fOCVVolt = (float)ParamOZ8806OCV.phydata;
			}
			else
			{
			...
			if ((ParamOZ8806PoOCV != null) && (ParamOZ8806SleepOCV != null))
			{
				if (ParamOZ8806PoOCV.phydata != 0)
				{
					//sRam[(int)prShortIndex.iOCVbit] = 1;
					batt_info.bPoOCV = true;
					batt_info.bSleepOCV = false;
				}
				else if (ParamOZ8806SleepOCV.phydata != 0)
				{
		...
		private bool GetCtrlValue(ref byte yRet)
		{
			...
			//if (ParamOZ8806Status != null)
			if((ParamOZ8806CtrlBI != null) && (ParamOZ8806Ctrlvme != null) && /*(ParamOZ8806CtrlChgActive != null) &&*/
				(ParamOZ8806CtrlSlpOCVEn != null) && (ParamOZ8806CtrlSleepMode != null) && (ParamOZ8806CtrlSWReset != null))
			{
				//sRam[(int)prShortIndex.iStatus] = (short)ParamOZ8806Status.phydata;
				yData = (byte)ParamOZ8806CtrlBI.phydata;
				yData |= (byte)((int)ParamOZ8806Ctrlvme.phydata << 2);
				//yData |= (byte)(ParamOZ8806CtrlChgActive.phydata << 4);
				yData |= (byte)((int)ParamOZ8806CtrlSlpOCVEn.phydata << 5);
				yData |= (byte)((int)ParamOZ8806CtrlSleepMode.phydata << 6);
				yData |= (byte)((int)ParamOZ8806CtrlSWReset.phydata << 7);
			}
			else
			{
				yData = 0;
		...
		private bool SetCtrlValue(byte yData)
		{
			...
			if ((ParamOZ8806CtrlBI != null) && (ParamOZ8806Ctrlvme != null) && 
				(ParamOZ8806CtrlSlpOCVEn != null) && (ParamOZ8806CtrlSleepMode != null) && (ParamOZ8806CtrlSWReset != null))
			{
				//sRam[(int)prShortIndex.iStatus] = (short)ParamOZ8806Status.phydata;
				//ParamOZ8806CtrlBI.phydata = (float)(yData & 0x01);		//it is readonly, should not try to modify it
				ParamOZ8806Ctrlvme.phydata = (float)((yData & 0x0C) >> 2);
				//ParamOZ8806CtrlChgActive.phydata = (float)((yData & 0x10) >> 4);
				ParamOZ8806CtrlSlpOCVEn.phydata = (float)((yData & 0x20) >> 5);
				ParamOZ8806CtrlSleepMode.phydata = (float)((yData & 0x40) >> 6);
				ParamOZ8806CtrlSWReset.phydata = (float)((yData & 0x80) >> 7);
				ParamOZ8806CtrlBI.errorcode = LibErrorCode.IDS_ERR_SBSSFL_REQUIREWRITE;
				ParamOZ8806Ctrlvme.errorcode = LibErrorCode.IDS_ERR_SBSSFL_REQUIREWRITE;
				//ParamOZ8806CtrlChgActive.errorcode = LibErrorCode.IDS_ERR_SBSSFL_REQUIREWRITE;
				ParamOZ8806CtrlSlpOCVEn.errorcode = LibErrorCode.IDS_ERR_SBSSFL_REQUIREWRITE;
				ParamOZ8806CtrlSleepMode.errorcode = LibErrorCode.IDS_ERR_SBSSFL_REQUIREWRITE;
				ParamOZ8806CtrlSWReset.errorcode = LibErrorCode.IDS_ERR_SBSSFL_REQUIREWRITE;
			}
			else
			{
				uGGErrorcode = LibErrorCode.IDS_ERR_SBSSFL_GGDRV_NOCTRLSTATUS;
		...
		private void bmu_init_chip()
        {
            ...
            gas_gauge.charge_end_current_th2 = (int)(myProject.dbChgEndCurr + 2);
            ...
            //gas_gauge.dsg_end_voltage_hi = SHUTDOWN_HI;   //Jun. version
            ...
            //gas_gauge.overflow_data = (Int32)((32768 * 5) / myProject.dbRsense);
			gas_gauge.overflow_data = (Int32)((32768 * 5) / (myProject.dbRsense * 1000));

            ////bmu_init_gg();

			bmu_printk(string.Format("AAAA O2MICRO OZ8806 DRIVER VERSION is {0}", VERSION));
			...
			bmu_printk(string.Format("O2MICRO OZ8806 test parameter  {0:F2},{1:F2},{2:F2},{3:F2},{4:F2},{5:F2},{6:F2},{7:F2},{8:F2},{9:F2},{10:d},{11:d},",
            ...
            //check_oz8806_staus();     //check OZ8806 i2c address, think we don't need it, done by DEM+BusOption
            //check_pec_control();

            //oz8806_create_sys();

            //afe_register_read_word(OZ8806_OP_BOARD_OFFSET, (uint16_t*)&data);
            if (ParamOZ8806BoardOffset != null)
            {
                data = (int)ParamOZ8806BoardOffset.phydata;
            }
            else
            {
                data = 0;
            }
            if (parameter_customer.debug) bmu_printk(string.Format("AAAAAAAAAAAAA regeidter is  %x", data)); 

            //ret = afe_read_board_offset(&data);
			//printk("AAAA board_offset is  {0}", data);
            ...
            if ((data > 10) || (data <= 0))
            {
                ...
                data = 7;
                WriteRegFunction(ParamOZ8806BoardOffset, data);
            }
            //}
            ...
            //if (ParamOZ8806Status != null)
                //i = (int)ParamOZ8806Status.phydata;
            //else 
                //i = 0;
			i = 0;
			...
			bmu_printk(string.Format("oz8806_cell_num  is  {0:d}", oz8806_cell_num));
            ...
            if (oz8806_cell_num > 1)
                ...
                i = 0x20;
            //WriteRegFunction(ParamOZ8806Status, i);
			SetCtrlValue((byte)i);

			bmu_printk(string.Format("O2MICRO OZ8806 test parameter  {0:F2},{1:F2},{2:F2},{3:F2},{4:F2},{5:F2},{6:F2},{7:F2},{8:F2},{9:F2},{10:F2},{11:F2},",
				(myProject.dbRsense * 1000),
                myProject.dbPullupR,
                myProject.dbPullupV,
                2.5F,//myProject.dbCARLSB,
        ...
        private void wait_ocv_flag_fun()
        {
			...
			CopyPhysicalToRam();
			GetOCVValue();
			bmu_printk(string.Format("read batt_info.fVolt is {0:F2}", (batt_info.fVolt * oz8806_cell_num)));
			...
			if(batt_info.bPoOCV)
			{
				...
				//CopyPhysicalToRam();	//don't need to do it again
			...
			//bmu_printk(string.Format("O2micro regeidter 0x00 is %x", i));

			//afe_register_read_byte(num_9, &i);
			//bmu_printk(string.Format("O2micro regeidter 0x09 is %x", i));	
		...
		private void trim_bmu_VD23()
		{
			...
			if ((ParamOZ8806Regx03Bit7 != null) && (ParamOZ8806Regx04Bit0 != null))
			{
				yVal = (byte)ParamOZ8806Regx03Bit7.phydata;
				yVal *= 2;
				yVal += (byte)ParamOZ8806Regx04Bit0.phydata;
			}

			WriteRegFunction(ParamOZ8806Regx08Bit0, (float)yVal);
		}
		...
		private void check_shutdown_voltage()
		{
			...
			bmu_printk(string.Format("Sleep data is {0:F2}", temp));
		...
		//no need
		private void check_oz8806_staus()
		{
		}
        ...
        private float calculate_soc_result()
        {
	        ...
	        else
            {
                ...
                //if (!rc_result)
                {
                    ...
                    mah_temp = infCal;
					if (mah_temp > myProject.dbDesignCp) mah_temp = myProject.dbDesignCp;
		...
		private void bmu_wait_ready()
		{
			...
			if(((i & 0x40)!=0) || (i == 0))
			{
				if (oz8806_cell_num > 1)
				{
					...
					//WriteRegFunction(ParamOZ8806Status, 0x2c);
					SetCtrlValue(0x2c);
				}
				else
				{
					//afe_register_write_byte(OZ8806_OP_CTRL,0x20);
					//WriteRegFunction(ParamOZ8806Status, 0x20);
					SetCtrlValue(0x20);
				}
				bmu_printk("oz8806 wake up function");
			}
			//ret = afe_register_read_byte(OZ8806_OP_CTRL,&i);
			...
			bmu_printk(string.Format("fVolt is {0:F2}", (batt_info.fVolt * oz8806_cell_num)));
...
//file_fail:
        ...
        private void wait_ready_code_after_file_fail()
        {
	        ...
	        if((data < 0)||(fcc < 0)|| (gas_gauge.ocv_flag<0)){
		        ...
		        if(waitready_times > 5)
                {
					...
					bmu_printk(string.Format("AAAA batt_info->fVolt is {0:F2}", (batt_info.fVolt * oz8806_cell_num)));
        ...
        private void bmu_power_down_chip()
        {
	        ...
	        if(oz8806_cell_num > 1)
            {
                ...
                //WriteRegFunction(ParamOZ8806Status, 0x6C);
				SetCtrlValue(0x6c);
            }
	        else
            {
		        //afe_register_write_byte(OZ8806_OP_CTRL,SLEEP_MODE | SLEEP_OCV_EN);
                //WriteRegFunction(ParamOZ8806Status, 0x60);
				SetCtrlValue(0x60);
            }

	        if(parameter_customer.debug)
            {
		        bmu_printk("eeee power down  oz8806.");
				bmu_printk(string.Format("eeee batt_info->fVolt is {0:F2}", (batt_info.fVolt * oz8806_cell_num)));
        ...
        private void bmu_wake_up_chip()
        {
            ...
            {
                ret = 0;
                //data = (byte)ParamOZ8806Status.phydata;
            ...
            if (batt_info.fRSOC >= 100)
            {
                ...
                if (batt_info.sCaMAH > (gas_gauge.fcc_data + gas_gauge.fcc_data *0.01F - 1))
                {
					bmu_printk(string.Format("oz8806 wake up batt_info.sCaMAH big error is {0:d}", batt_info.sCaMAH));
        ...
        private void check_board_offset()
        {
	        ...
	        if(parameter_customer.board_offset != 0)
	        {
                ...
                WriteRegFunction(ParamOZ8806BoardOffset, parameter_customer.board_offset);
		        check_offset_flag = 0;
		        return;
	        }
	        ...
	        else
	        {
                ...
                if (((offset - data) > 2) || ((offset - data) < -2))
                {
                    ...
                    WriteRegFunction(ParamOZ8806BoardOffset, offset);
                }
	        }
        ...
        private void bmu_polling_loop()
        {
            ...
            if (parameter_customer.debug)
				bmu_printk(string.Format("CAR is {0:d}, error range = {1:F2}", data, (myProject.dbDesignCp * 0.012F)));

            ret = 0;
            if ((ret >= 0) && (data > 0))
            {
                // for big current charge
                if (((batt_info.fRCPrev - data) < (myProject.dbDesignCp * 0.012F)) &&
                    ((data - batt_info.fRCPrev) < (myProject.dbDesignCp * 0.012F)))
                {
                    batt_info.fRC = data;
                    polling_error_times = 0;
                }
                else
                {
                    ...
                    if (polling_error_times > 3)
                    {
						...
						bmu_printk(string.Format("Write sCaMAH= {0:d} to CAR", batt_info.sCaMAH));
            ...
            if (parameter_customer.debug)
            {
				bmu_printk(string.Format("first sCaMAH:{0:F2},fRC:{1:F2},fRCPrev:{2:F2},fcurr:{3:F2},volt:{4:F2}, RSOC: {5:F2}\n",
                    batt_info.sCaMAH, batt_info.fRC, batt_info.fRCPrev,
                    batt_info.fCurr, (batt_info.fVolt * oz8806_cell_num), batt_info.fRSOC));
            }
            //back sCaMAH
            ...
            if (parameter_customer.debug)
				bmu_printk(string.Format("second sCaMAH:{0:d},fRC:{1:F2},fRCPrev:{2:F2},fcurr:{3:F2},volt:{4:F2}, RSOC:{5:F2}\n",
                    batt_info.sCaMAH, batt_info.fRC, batt_info.fRCPrev,
                    batt_info.fCurr, (batt_info.fVolt * oz8806_cell_num), batt_info.fRSOC));
	        ...
	        */
            ...
            if (batt_info.sCaMAH > (gas_gauge.fcc_data + gas_gauge.fcc_data * 0.01F - 1))//(full_charge_data))
            {
				bmu_printk(string.Format("big uppder error sCaMAH is {0:d}", batt_info.sCaMAH));
                batt_info.sCaMAH = (int)(gas_gauge.fcc_data + gas_gauge.fcc_data * 0.01F - 1);//full_charge_data;
            }

            if (batt_info.sCaMAH < (gas_gauge.fcc_data * 0.01F -1)) // 100 - 1))
            {
				bmu_printk(string.Format("big lower error sCaMAH is {0:d}",batt_info.sCaMAH));
            ...
            {
				bmu_printk(string.Format("fcc uppper error is {0:d}", gas_gauge.fcc_data));
            ...
            {
				bmu_printk(string.Format("fcc lower error is {0:d}", gas_gauge.fcc_data));
	        ...
	        if(data >= 0)
	        {
		        if(fRSOC_PRE != batt_info.fRSOC)
		        {
			        ...
			        return;		//return bmu_polling_loop();
			...
			if(GetCtrlValue(ref i))
            {
                //i = (byte)(int)ParamOZ8806Status.phydata;
				//i = (byte)sRam[(int)prShortIndex.iStatus];
            }
            else
            {
	        ...
	        if(ret >= 0)
	        {
		        if((data > 10) || (data <= 0))
		        {
					bmu_printk(string.Format("oz8806 board offset error is {0:d}", data));
			        //afe_write_board_offset(7);
                    data = 7;
                    WriteRegFunction(ParamOZ8806BoardOffset, data);
		        }
        ...
        private void oz8806_over_flow_prevent()
        {
            Int32 ret;
            Int32 data;
        ...
        private void charge_end_process()
        {
            ...
            batt_info.sCaMAH = (int)(gas_gauge.fcc_data + gas_gauge.fcc_data * 0.01 - 1);//full_charge_data; ;
            bmu_write_data(BATT_CAPACITY, batt_info.sCaMAH);
        ...
        private void charge_process()
        {
            ...
            if (gas_gauge.charge_table_flag == 0)
            {
                ...
                if (rc_result == 0)
                {
                    ...
                    if ((estimate_capacity <= 0) || (batt_info.sCaMAH >= gas_gauge.fcc_data))
                        ...
                        gas_gauge.charge_ratio = (int)(1000F * (gas_gauge.fcc_data - batt_info.sCaMAH) / estimate_capacity);
                ...
                {
                    gas_gauge.charge_ratio = (int)gas_gauge.charge_max_ratio;
                }
            ...
            if (capacity > 0)
            {
				...
				fRSOCbk = batt_info.fRSOC;		//(A150902)Francis, back up previous RSOC
                batt_info.fRSOC = batt_info.sCaMAH * 100;
                batt_info.fRSOC = batt_info.fRSOC / gas_gauge.fcc_data;
                ...
                if ((gas_gauge.ocv_flag != 0)&& (batt_info.fCurr >= gas_gauge.charge_end_current_th2))
                {
                    if (batt_info.fRSOC >= 100)
                    {
                        batt_info.fRSOC = fRSOCbk;		//(A150902)Francis, then resume it
                        batt_info.sCaMAH = gas_gauge.fcc_data - 1;
                    }
                }
                else if (batt_info.fRSOC >= 100)
		...
		private void discharge_process()
		{
				...
				if(rc_result == 0)
				{
					...
					*/
					...
					if(infCal <= 0)
						...
						gas_gauge.discharge_ratio = (int)(1000 * batt_info.sCaMAH / infCal + 0.5F);
					if(parameter_customer.debug)
					{
					...
					*/
					...
					{
						gas_gauge.discharge_ratio = (int)gas_gauge.discharge_max_ratio;
					}
				}
			}		//if((gas_gauge.discharge_table_flag == 0) && (batt_info.fVolt < (myProject.dbChgCVVolt - 100)))
			...
			*/
			...
			if(capacity > 0)
			{
				...
				else             //if(gas_gauge.discharge_table_flag != 0)
				{
					batt_info.sCaMAH -=  (int)(capacity + 0.5F);
				}
