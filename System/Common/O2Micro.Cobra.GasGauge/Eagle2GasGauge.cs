...
namespace O2Micro.Cobra.GasGauge
{
	...
	public class Eagle2GasGauge : GasGaugeInterface
	{
		#region private enum
...
//		#region private methods
		...
		private bool daq_adc_data_read()
		{
			...
			//go belows
			{
						...
						case GGType.OZCAR:
							{
								...
								{
                                    /********************************************************///leon: 防止CAR跳变
                                    //if ((ftmp - fRam[(byte)prFloatIndex.iCAR]) > 25)
                                        //ftmp = fRam[(byte)prFloatIndex.iCAR] + 25;
                                    //else if ((ftmp - fRam[(byte)prFloatIndex.iCAR]) < -25)
                                        //ftmp = fRam[(byte)prFloatIndex.iCAR] - 25;
                                    /********************************************************/
									fRam[(byte)prFloatIndex.iCarDiff] = ftmp - fRam[(byte)prFloatIndex.iCAR];
                ...
                float i = fRam[(int)prFloatIndex.iCurrent];
                float r = myProject.dbRcon;
                //OCVData = fRam[(int)prFloatIndex.iVoltage] - fRam[(int)prFloatIndex.iCurrent] * (myProject.dbRcon + myProject.dbRbat);
                fRam[(int)prFloatIndex.iVoltage] = fRam[(int)prFloatIndex.iVoltage] - i * r;
				...
				if (bSeaElfSupport)
				{
					...
					if (bSeaElfSupport)
					{
                        ...
                        {
                            UInt16 temp = ((UInt16)ChargerReg.CHARGERFULLYCHARGED);
                            temp = (UInt16)~temp;
                            yValue &= (byte)temp;
                        }
...
//					}
		...
		private void gg_init()
		{
			...
			if (bGInit)
			{
				...
				//if there is big difference between OCV and voltage use current voltage to initialize CAR, instead of OCV
				if (Math.Abs(fRam[(int)prFloatIndex.iOCVolt] - fRam[(int)prFloatIndex.iVoltage]) > 20)
				{
					if (fRam[(int)prFloatIndex.iVoltage] > 2000)		//(A150528)Francis, add for unknow why voltage will be minus value at first time read
					{
					...
					//fRam[(byte)prFloatIndex.iCAR] = (float)(dbPercent * 0.01 * myProject.dbDesignCp);
					////fRam[(int)prFloatIndex.iPercent] = dbPercent;		//percentage is calculated in sbd_update_sbs()
					////fRam[(byte)prFloatIndex.iCAR] = (float)((dbPercent /32768) * myProject.dbDesignCp);
					//if (fRam[(byte)prFloatIndex.iCAR] > myProject.dbDesignCp)
					//{
						//fRam[(byte)prFloatIndex.iCAR] = myProject.dbDesignCp;
					//}
					//fRam[(int)prFloatIndex.iCAMah] = fRam[(byte)prFloatIndex.iCAR];
					//fRam[(int)prFloatIndex.iPrevCAMah] = fRam[(byte)prFloatIndex.iCAR];
				}
                //leon: consider Rcon and Rbat
                if (Math.Abs(fRam[(int)prFloatIndex.iCurrent]) > 30)
                {
                    float i = fRam[(int)prFloatIndex.iCurrent];
                    float r = myProject.dbRcon + myProject.dbRbat;
                    //OCVData = fRam[(int)prFloatIndex.iVoltage] - fRam[(int)prFloatIndex.iCurrent] * (myProject.dbRcon + myProject.dbRbat);
                    OCVData = OCVData - i * r;
                }
                fRam[(int)prFloatIndex.iOCVolt] = OCVData;
				...
				fRam[(int)prFloatIndex.iCTMah] = fRam[(byte)prFloatIndex.iCAR];
				fRam[(int)prFloatIndex.iCAMah] = fRam[(byte)prFloatIndex.iCAR];
				fRam[(int)prFloatIndex.iPrevCAMah] = fRam[(byte)prFloatIndex.iCAR];
				fRam[(byte)prFloatIndex.iCAR] = 0;
		...
		private void gg_update_fctmah()
		{
            ...
            if (Math.Abs(fRam[(int)prFloatIndex.iCarDiff]) < 15)
		...
		private void gg_self_discharge(int iHour)
		{
			...
			fRam[(int)prFloatIndex.iPrevCAMah] = fRam[(int)prFloatIndex.iCAMah];*/  //leon: no need to do this in cobra
		}
		...
		private void sbd_udpate_sbs()
		{
			...
			if (ParamSBS0x0D != null)
			{
				...
				//fTempsbd = (100 * fRam[(byte)prFloatIndex.iCAR]) / fTempsbd;
			...
			if (ParamSBSxF5SelfMah != null)
			{
				...
				//ParamSBSxF5SelfMah.phydata = fRam[(byte)prFloatIndex.iCAR];
				ParamSBSxF5SelfMah.phydata = Convert.ToDouble(String.Format("{0:F2}", fRam[(byte)prFloatIndex.iCAR]));
			}
			//SBSxF6
			...
			if (sRam[(int)prShortIndex.iOneMinTmr] <= 0)
			{
				...
				if (((ushort)sRam[(int)prShortIndex.iBGStatus] & (ushort)BGStatusenum.BG_STATUS_IDLE_2HOUR) != 0)
				{
					...
					else
					{
						...
						{
							fRam[(int)prFloatIndex.iOCVCompsate] = fcomp;
							fRam[(byte)prFloatIndex.iCAR] += fdelta;			//add into CC
		...
		private void age_compensate_rc_lookup()
		{
			...
			age_comp.abs_cap_now = fRam[(byte)prFloatIndex.iCAR];// batt_info->fRC;	//set CAR value if table lookup failed  
		}
		...
		*/
		...
		#region main() simulation, actually this is timer function, run every 4 second
