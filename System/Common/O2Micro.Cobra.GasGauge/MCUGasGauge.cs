...
namespace O2Micro.Cobra.GasGauge
{
	...
	public class MCUGasGauge : GasGaugeInterface
	{
		#region private enum
		...
		private enum prShortIndex : byte
		{
			...
			iBatteryType			= 0x1E,			//save Battery Voltage from MCU
		...
		private byte yMCUSebSec = 1;			//subsection definition, same as XML, used in DEM
		private byte yBatterySubSec = 2;		//subsection definition, same as XML, used in DEM
		private byte yChargerSubSec = 3;		//sunsection definition, same as XML, used in DEM
		private byte yMCUParameter = 2;		//document section 8.2
		private byte yMCUSBSMonitor = 3;	//document section 8.3
		private byte yMCUPlatform = 4;			//document section 8.4
		...
		//for MCU used only
		private Parameter ParamSBSMCUx60 = null;
		private Parameter ParamSBSMCUx63 = null;
		private Parameter ParamSBSMCUx65 = null;
		private Parameter ParamSBSMCUx67 = null;
		private Parameter ParamSBSMCUx69 = null;
		private Parameter ParamSBSMCUx6B = null;
		private Parameter ParamSBSMCUx6D = null;
		private Parameter ParamSBSMCUx6F = null;
		private Parameter ParamSBSMCUx71 = null;
		private Parameter ParamSBSMCUx72 = null;
		#endregion
		...
		
		{
				...
				(PListGGPolling != null) && (PListGGSetting != null) && (PListSBSReg != null))
			{
				...
				//WriteChipsReg(yBatterySubSec, 0x08, 0x02, 0x01);	//force PEC enable
				...
				tmrFirmware.Interval = TimeSpan.FromMilliseconds(2000);
...
//		#region private methods
		...
		private bool bg_init()
		{
			...
			{
				if (((pmSBStmp.guid & 0x0000FFFF) >> 8) == (byte)(EGSBS.SBSMCUChagerBoardRunTime))
				{	//use 0x63 as Debug paramter
		...
		private bool gdm_init()
		{
			...
			foreach (Parameter pmGDM in PListSBSReg)
			{
					...
					switch (u16Addr)
					{
						case (UInt16)EGSBS.SBSMCUBatteryVoltage:
							{
								ParamSBS0x3C = pmGDM;
								ParamSBSMCUx6B = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUBatteryCurrent:
							{
								ParamSBS0x0A = pmGDM;
								ParamSBSMCUx6D = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUBatteryTemp:
							{
								ParamSBS0x4A = pmGDM;
								ParamSBSMCUx6F = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUBatteryCapacity:
							{
								ParamSBS0x0F = pmGDM;
								ParamSBSMCUx71 = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUBatterySoC:
							{
								ParamSBS0x0D = pmGDM;
								ParamSBSMCUx72 = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUChargerControl:
							{
								ParamSBSMCUx60 = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUChagerBoardRunTime:
							{
								ParamSBSMCUx63 = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUStatus:
							{
								ParamSBSMCUx65 = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUChargerStatus:
							{
								ParamSBSMCUx67 = pmGDM;
								break;
							}
						case (UInt16)EGSBS.SBSMCUVBusVoltage:
							{
								ParamSBSMCUx69 = pmGDM;
								break;
							}
			...
			if (ParamSBS0x19 != null)
			{
				...
				ParamSBS0x19.phydata = Convert.ToDouble(String.Format("{0:F2}", sRam[(int)prShortIndex.iBatteryType]));
			}
		...
		private bool afe_detect_device()
		{
			...
			bAFE = ReadChipsByte(yBatterySubSec, ref regID, ref yResult, true);
			if (bAFE)
			{
		...
		private bool daq_adc_data_read()
		{
			...
			//go belows
			{
				...
				if (bSeaElfSupport)
				{
					...
					if (bSeaElfSupport)
					{
						bPoll = ReadChipsByte(yChargerSubSec, ref SeaElfSatusReg, ref yValue, true);
					}
					else
					{
						bPoll = true;
				...
				//OVP,UVP bit set
				if ((yValue & (byte)VBusSysReg.VSYSOVP) != 0)
				{
					//OVP, no OVP flag?? TBD
...
//					}
		...
		private void gg_init()
		{
			...
			if (bGInit)
			{
				...
				//(A150507)Francis, read MCU 0x18 for Battery Type
				//MCUReg.address = 0x18;
				MCUReg.address = 0x04;
				MCUReg.startbit = 0;
				MCUReg.bitsnumber = 0x10;
				bGInit = ReadChipsWord(yMCUSebSec, ref MCUReg, ref wData);
				sRam[(int)prShortIndex.iBatteryType] = 4200;
				if (bGInit)
				{
					if (wData == 0x0001)
						sRam[(int)prShortIndex.iBatteryType] = 4350;
				}
				//(E150507)
		...
		private void sbd_udpate_sbs()
		{
			...
			if(ParamSBS0x15 != null)
			{
				...
				ParamSBS0x15.phydata = Convert.ToDouble(String.Format("{0:F2}", sRam[(int)prShortIndex.iBatteryType]));
			}
			...
			if (ParamSBSMCUx60 != null)
			{
				ParamSBSMCUx60.phydata = Convert.ToDouble(String.Format("{0:F2}", 0));
			}

			if (ParamSBSMCUx63 != null)
			{
				short ustate = (short)sRam[(int)prShortIndex.iBGState];
				float femp = 65535;
				...
				//ParamSBS0x13.phydata = femp;
				ParamSBSMCUx63.phydata = Convert.ToDouble(String.Format("{0:F2}", femp));
			}

			if (ParamSBSMCUx65 != null)
			{
				ParamSBSMCUx65.phydata = Convert.ToDouble(String.Format("{0:F2}", sRam[(int)prShortIndex.iSeaElfRegx40]));
			}

			if (ParamSBSMCUx67 != null)
			{
				Double dtmp;
				dtmp = sRam[(int)prShortIndex.iSeaElfRegx41];
				dtmp *= 65536;
				dtmp += sRam[(int)prShortIndex.iSeaElfRegx42];
				ParamSBSMCUx67.phydata = Convert.ToDouble(String.Format("{0:F2}", dtmp));
			}

			if (ParamSBSMCUx69 != null)
			{
				ParamSBSMCUx69.phydata = Convert.ToDouble(String.Format("{0:F2}", sRam[(int)prShortIndex.iBatteryType]));
			}
		...
		*/
		...
		private bool GetOCVdata(ref float fOCVmV)
		{
			...
			bOCV = ReadChipsShort(yBatterySubSec, ref addrLo, ref addrHi, ref fOCVmV);
		...
		private bool GetPoOCVFlag(ref byte yFlag)
		{
			...
			bFlag = ReadChipsByte(yBatterySubSec, ref regOCV, ref yFlag, true);
		...
		private bool AccessCtrlReg(bool bWriteR, bool bReadR, ref byte yValData)
		{
			...
			if (bWriteR)
			{
				bCtrl = WriteChipsReg(yBatterySubSec, ref regCtrl, yValData, true);
			}
			else if (bReadR)
			{
				bCtrl = ReadChipsByte(yBatterySubSec, ref regCtrl, ref yValData, true);
			}
		...
		private bool AccessPECControl(bool bWriteP, bool bReadP, ref byte yValPEC)
		{
			...
			if (bWriteP)
			{
				bPEC = WriteChipsReg(yBatterySubSec, ref regPEC, yInput, true);
			}
			else if (bReadP)
			{
				bPEC = ReadChipsByte(yBatterySubSec, ref regPEC, ref yInput, true);
			}
		...
		private bool AccessBoardCurrent(bool bWriteA, bool bReadA, ref float fValData)
		{
			...
			if (bWriteA)
			{
				...
				bBdoff = WriteChipsFloat(yBatterySubSec, ref regBoardoffLo, ref regBoardoffHi, fValData);
				//bBdoff = WriteChipsReg(yBatterySubSec, ref regBoardoffLo, (byte)((byte)fValData & 0xFF), true);
			}
			else if (bReadA)
			{
				bBdoff = ReadChipsShort(yBatterySubSec, ref regBoardoffLo, ref regBoardoffHi, ref fValData);
			}
		...
		private bool AccessCARReg(bool bWriteA, bool bReadA, ref float fValData)
		{
			...
			if (bWriteA)
			{
				bCAR0 = WriteChipsFloat(yBatterySubSec, ref regCARLo, ref regCARHi, fValData);
			}
			else if (bReadA)
			{
				bCAR0 = ReadChipsShort(yBatterySubSec, ref regCARLo, ref regCARHi, ref fValData);
			}
		...
		private bool WriteChipsFloat(byte yChip, ref Reg regLo, ref Reg regHi, float fData)
		{
			...
			else
			{
				...
				devParent.bBusy = false;
				//bWrite = true;
				ReadChipsShort(yBatterySubSec, ref regLo, ref regHi, ref fBack);
		...
		private bool ReadChipsWord(byte yChip, ref Reg regTart, ref UInt16 wData, bool bReadAll = false)
		{
			bool bRead = false;
			bool bFound = false;
			ParamContainer pmcTemp = new ParamContainer();
			...
			else
			{
				...
				//go belows
				{
					...
					if ((UInt16)pamTemp.phydata < 0xFF)
					{
						wData = Convert.ToUInt16(pamTemp.phydata);
						bRead = true;
					}
					else
					{
		...
		private bool WriteChipsWord(byte yChip, ref Reg regTart, UInt16 wData, bool bWriteAll = false)
		{
			bool bWrite = false;
			bool bFound = false;
			ParamContainer pmcTemp = new ParamContainer();
