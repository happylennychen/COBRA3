...
namespace O2Micro.Cobra.Common
{
    /*public class CobraPrj
    {
        ...
        private AsyncObservableCollection<Project> m_prjlist = new AsyncObservableCollection<Project>();
        public AsyncObservableCollection<Project> prjlist    //LogData的集合
        {
            get { return m_prjlist; }
            set
            {
                m_prjlist = value;
                //OnPropertyChanged("prjlist");
            }
        }

        public CobraPrj(string str)  //指定foldername的构造函数
        {
            folder = str;
        }
    ...
    static public class Project        // : INotifyPropertyChanged
    {
        ...
        private List<string> m_settingfiles = new List<string>();
        public List<string> settingfiles
        {
            get { return m_settingfiles; }
            set { m_settingfiles = value; }
        }*/

        static List<string> LoadPrj(string prjname, string destination)  //解压prj，将解压后的文件放入指定目录，返回解压后的文件名
        {
            List<string> settingfiles = new List<string>();
            return settingfiles;
        }

        static bool SavePrj(List<string> settingfiles)  //将若干个setting file压缩到一个prj文件中
        {
            return true;
        }
    }

	#region SBS GasGauge project setting/class
	...
	public class GasGaugeProject
	{
		...
		{
			/*
			string ggfolder = System.IO.Path.Combine(FolderMap.m_currentproj_folder, "OZBattery\\Project\\");
			if (!Directory.Exists(ggfolder))
				Directory.CreateDirectory(ggfolder);
			ggfolder = System.IO.Path.Combine(ggfolder, filename);
			if (File.Exists(ggfolder))
				strFileProject = ggfolder;
			else
				strFileProject = null;
			 */
			...
			//assing a default value, just for case that xml has no definition
			strFileProject = null;
			strFileGGDll = null;
